...
TComplex myPDF::LASSParam(Double_t RMassCalc, Double_t Dau2Mass, Double_t MomMass, Double_t DB0) const
{
    Double_t PRMassCalc = dec2mm_host(MomMass,RMassCalc,Dau2Mass);
    Double_t QRMassCalc = dec2mm_host(RMassCalc,MKaon,MPion);
    //Double_t PK_0_1430 = dec2mm_host(MomMass,M1430_0,Dau2Mass);
    Double_t QK_0_1430 = dec2mm_host(M1430_0,MKaon,MPion);
    Double_t PLASS = dec2mm_host(MomMass,1.0,Dau2Mass);
    //Double_t PLASS = dec2mm_host(MomMass,M800,Dau2Mass); //alternate mass for low mass S wave

    //Double_t thetaB = TMath::ATan( 1.0/( 1.0/(a_LASS * PRMassCalc) + 0.5*b_LASS * PRMassCalc ) );
    Double_t thetaB = TMath::ATan( 1.0/( 1.0/(a_LASS * QRMassCalc) + 0.5*b_LASS * QRMassCalc ) );
    //TComplex NRterm = RMassCalc/PRMassCalc * TMath::Sin(thetaB) * TComplex::Exp(TComplex::I()* thetaB ) ;
    TComplex NRterm = RMassCalc/QRMassCalc * TMath::Sin(thetaB) * TComplex::Exp(TComplex::I()* thetaB ) ;
    //Double_t gammaR = G1430_0 * (PRMassCalc/PK_0_1430) * (M1430_0/RMassCalc);
    Double_t gammaR = G1430_0 * (QRMassCalc/QK_0_1430) * (M1430_0/RMassCalc);
    TComplex BWdenom = M1430_0*M1430_0 - RMassCalc*RMassCalc - TComplex::I()*M1430_0*gammaR;
    //TComplex BWterm = 2.0 * TComplex::Exp(TComplex::I()* 2.0 * thetaB) * G1430_0 * (M1430_0*M1430_0/PK_0_1430) / BWdenom;
    TComplex BWterm = 2.0 * TComplex::Exp(TComplex::I()* 2.0 * thetaB) * G1430_0 * (M1430_0*M1430_0/QK_0_1430) / BWdenom;
    TComplex LASSterm = BlattWeisskopf(1, PRMassCalc, PLASS, DB0) * (PRMassCalc/MomMass) * (NRterm + BWterm);
    return LASSterm;
}
/*
  ...
  */
  ...
  if (Ks_check == kTRUE) {
    ...
    for (Int_t iKstar_S=0; iKstar_S<(Int_t)Kstar_spin.size(); ++iKstar_S) {
      ...
      if (LASS && iKstar_S==0) {
        matrixElement_R = LASSParam(mKP,MPsi_nS, MBd, dRadB0)*
